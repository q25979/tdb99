...
g_sell_order_status = TypedefConst()

g_sell_order_status.UNPROCESSED = 0  # 未处理
g_sell_order_status.PROCESSED = 1  # 处理成功
g_sell_order_status.FAILED = 2  # 处理失败


class SellOrder(AutoIncrementBase):
    user_id = db.Column(db.String(36), db.ForeignKey('user.id'), nullable=False)  # 用户
    side = db.Column(db.SmallInteger, default=2, nullable=False)  # 单类型 参见 g_order_side
    ...
    status = db.Column(db.SmallInteger, default=g_sell_order_status.UNPROCESSED)  # 状态参见 g_sell_order_status
    ...
    def order_create(user, amount):
        if user.transaction_level != g_user_transaction_level.ULTIMATE:
            order = SellOrder.query.filter(SellOrder.user_id == user.id,
        ...
        order = SellOrder(user_id=user.id, amount=amount)
        return True, order
