<template>
  <main-container :title="$t('common.total_asset_wallet')" :hasTabbar="false" containerColor="#f7f7f7">
     <mt-button icon="back" slot="left" @click.native="$router.go(-1)"></mt-button>
    <div class="asset-general">
      <div class="asset-total">
        ...
        <div class="total-bottom">
          <div class="total-content">
            <p class="total-title">{{$t('common.super_vip')}}</p>
            <p class="total-count">0</p>
          </div>
          <div class="total-content" @click="$router.push('/finance/node_profit')">
             <p class="total-title">{{$t('common.node_profit')}}</p>
            <p class="total-count">{{memberProfile.node_profit}}</p>
          </div>
        </div>
      </div>
      </div>
...
export default {
  ...
  mounted() {
    this.$get('/member/assets/query').then(res => {
      this.assetData = res.data.total_balance;
    });
    this.getList(true);
  },
  components: {
