<template>
  <main-container :title="$t('account.security_psd_title')" :hasTabbar="false" containerColor="#fff">
  ...
  methods: {
    handleSave() {
      ...
      this.$put('/member/user/current_user', {
        new_security_password: this.newPsd,
        old_security_password: this.oldPsd,
      })
      .then(resp => {
        this.newPsd = '';
        this.oldPsd = '';
