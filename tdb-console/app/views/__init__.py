...
class Api:
    ...
    def get(resource_path, params=None, response_type='json'):
        ...
        r = requests.get(Api.url(resource_path), params, timeout=(19, 19))
    ...
    def post(resource_path, data, response_type='json'):
        ...
        r = requests.post(Api.url(resource_path), json=data, timeout=(19, 19))
        return Api.handle_response(r, response_type)
    ...
    def put(resource_path, data):
        if session.get('token'):
            resource_path += '?token={}'.format(session.get('token'))
        r = requests.put(Api.url(resource_path), json=data, timeout=(19, 19))
        return Api.handle_response(r)

    @staticmethod
    def delete(resource_path):
        if session.get('token'):
            resource_path += '?token={}'.format(session.get('token'), timeout=(9, 9))
